...
| T-00-11 | Tampering | A future edit reintroduces a bad/unknown/shut-down model id | high | mitigate | Static allowlist check in CI asserts `MODEL_ID ∈ ALLOWLIST` on every push/PR (`scripts/check-model-allowlist.mjs`, `.github/workflows/ci.yml:35`) — no quota spent. **Corrected (`684c1af`, 2026-08-18):** the guardrail worked as designed but the *data* was wrong — CR-01 had removed the real `gemini-3.6-flash` and kept the now-deprecated `gemini-2.5-flash` (which 404s). Repinned to live-verified `gemini-3.6-flash`; allowlist rebuilt from the live API. The static check itself is unchanged and still closed. | closed |
